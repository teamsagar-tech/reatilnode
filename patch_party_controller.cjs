const fs = require('fs');
const file = '/Users/ratan/Downloads/RetailNodeV2/backend/controllers/partyController.js';
let content = fs.readFileSync(file, 'utf8');

const oldCreateMatch = /exports\.createParty = async \(req, res\) => \{[\s\S]*?res\.status\(500\)\.json\(\{ error: 'Internal Server Error' \}\);\n\s*\}\n\};/g;

const newCreate = `exports.createParty = async (req, res) => {
  const {
  ...
  try {
    const [result] = await db.execute(
      \`INSERT INTO Parties (
        ...
        account_name, bank_name, account_number, ifsc, branch, bank_account_type, gst_raw_data, categories, brands, brand_type
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)\`,
      [
        req.firm_id,
        gstin || null,
...
content = content.replace(oldCreateMatch, newCreate);

const oldUpdateMatch = /exports\.updateParty = async \(req, res\) => \{[\s\S]*?res\.status\(500\)\.json\(\{ error: 'Internal Server Error' \}\);\n\s*\}\n\};/g;

const newUpdate = `exports.updateParty = async (req, res) => {
  const { id } = req.params;
  const {
    gstin, panNumber, state, stateCode, partyName, shortName, type,
    line1, line2, line3, pincode, city, taluka, district,
...
content = content.replace(oldUpdateMatch, newUpdate);
