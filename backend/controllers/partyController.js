...
exports.createParty = async (req, res) => {
  const {
    ...
    categories, brands, brandType, contacts, invoiceConfig
  } = req.body;
  ...
  try {
    const [result] = await db.execute(
      `UPDATE Parties SET
        ...
        categories = ?, brands = ?, brand_type = ?, contacts = ?, invoice_config = ?
...
exports.updatePartyInvoiceConfig = async (req, res) => {
  const { id } = req.params;
  const { invoiceConfig } = req.body;
  try {
    const [result] = await db.execute(
      `UPDATE Parties SET invoice_config = ? WHERE id = ? AND firm_id = ?`,
      [invoiceConfig ? JSON.stringify(invoiceConfig) : null, id, req.firm_id]
    );
    if (result.affectedRows === 0) {
      return res.status(404).json({ error: 'Party not found or unauthorized' });
    }
    res.json({ message: 'Party invoice config updated successfully' });
  } catch (error) {
    console.error('Error updating party invoice config:', error);
    res.status(500).json({ error: 'Internal Server Error' });
  }
};
