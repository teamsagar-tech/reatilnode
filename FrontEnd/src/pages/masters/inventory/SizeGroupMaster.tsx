...
export default function SizeGroupMaster() {
  ...
  const [formData, setFormData] = useState<any>({ sizes: '' });
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Escape') {
        ...
        setFormData({ sizes: '' });
        setTimeout(() => {
          document.getElementById('field-groupName')?.focus();
        }, 50);
  ...
  const InputRow = ({ label, value, onChange, width = 'flex-1', type = 'text', placeholder = '', id = '' }: any) => (
  ...
  return (
    <>
      <Helmet>
        <title>Size Group Master | RetailNode ERP</title>
      </Helmet>
      ...
      <div className='flex flex-col h-screen font-sans text-[13px] selection:bg-transparent overflow-hidden bg-[#e0efeb] w-full'>
        <div className='flex flex-1 p-1 gap-1 overflow-hidden h-full'>
          ...
          <div className='flex-1 bg-[#fcfaf2] border-2 border-[#81a09d] flex flex-col overflow-hidden shadow-inner relative'>
            <div className='bg-[#1b5e58] text-white font-bold px-2 py-1 flex justify-between shrink-0'>
               ...
               <div className='text-yellow-300'>Size Group Master (Matrix Entry)</div>
            </div>
              ...
              {mode === 'list' ? (
                <>
                  <div className='flex justify-between items-center mb-2'>
                    ...
                    <button 
                      onClick={() => { setMode('create'); setFormData({ sizes: '' }); }} 
                  ...
                  <table className='w-full text-left border-collapse border border-slate-400'>
                    <thead className='bg-[#eef5ed]'>
                      <tr className='border-b-2 border-slate-400 text-slate-900 font-bold text-[12px]'>
                        ...
                        <th className="px-2 py-1 border-r border-slate-300">Group Name</th>
                        <th className="px-2 py-1 border-r border-slate-300">Sizes</th>
                        <th className="px-2 py-1 border-r border-slate-300">Status</th>
                      </tr>
                    </thead>
                    <tbody>
                      {sizeGroups.map((row, idx) => (
                        <tr key={row.id} className={'text-[12px] border-b border-slate-300 ' + (idx % 2 === 0 ? 'bg-white' : 'bg-[#fcfaf2]') + ' hover:bg-[#ffffe0] cursor-pointer'}>
                          ...
                          <td className="px-2 py-1 border-r border-slate-300 font-bold text-slate-900">{row.group_name}</td>
                          <td className="px-2 py-1 border-r border-slate-300 font-medium text-slate-700">{Array.isArray(row.sizes) ? row.sizes.join(', ') : '[]'}</td>
                          <td className="px-2 py-1 border-r border-slate-300 font-medium text-slate-700">{row.is_active ? 'Active' : 'Inactive'}</td>
                        </tr>
                      ))}
                      {sizeGroups.length === 0 && !loading && (
                        <tr><td colSpan={4} className="text-center py-4 text-slate-500">No size groups found</td></tr>
                      )}
                    </tbody>
                  </table>
                </>
                ...
                <div className='flex flex-col h-full overflow-hidden'>
                  <div className='flex flex-1 gap-6 overflow-hidden'>
                    ...
                    <div className="w-[50%] flex flex-col gap-1 border-r-2 border-slate-300 pr-4 overflow-y-auto pb-4 custom-scrollbar">
                      <SectionTitle>Size Group Information</SectionTitle>
                      <InputRow id="field-groupName" label="Group Name" placeholder="e.g. Momento Sizes (1 to 16)" value={formData.groupName || ''} onChange={(v: string) => setFormData({...formData, groupName: v})} />
                      
                      <div className="flex items-start mb-[2px] mt-2">
                        <div className="w-[110px] text-slate-800 font-bold text-[11px] text-right pr-2 pt-1 leading-tight">
                          Sizes<br/><span className="font-normal text-[9px] text-slate-500">(Comma separated)</span>
                        </div>
                        <textarea 
                          className={`bg-white border border-slate-400 px-1 py-[2px] text-[12px] font-bold text-black focus:bg-[#ffffe0] focus:outline-none focus:border-slate-800 flex-1 min-h-[60px] resize-none`}
                          value={formData.sizes}
                          onChange={e => setFormData({...formData, sizes: e.target.value})}
                          placeholder="e.g. 1, 2, 3, 4, 5, 6, 8, 10, 12, 14, 16"
                        />
                      </div>
                    </div>
                  </div>
        ...
        <div className='bg-[#1b5e58] text-white text-[11px] px-4 py-1 flex justify-between items-center border-t-2 border-[#12423d]'>
          <div className='font-medium tracking-wide'>Size Group Master</div>
        </div>
      </div>
    </>
  );
