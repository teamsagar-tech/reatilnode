...
export default function CustomerMaster() {
  ...
  return (
    <>
      <Helmet>
        <title>Customer Master | RetailNode ERP</title>
      </Helmet>
      ...
      <div className='flex flex-col h-screen font-sans text-[13px] selection:bg-transparent overflow-hidden bg-[#e0efeb] w-full'>
        ...
        <div className='flex flex-1 p-1 gap-1 overflow-hidden h-full'>
          ...
          <div className='flex-1 bg-[#fcfaf2] border-2 border-[#81a09d] flex flex-col overflow-hidden shadow-inner relative'>
            <div className='bg-[#1b5e58] text-white font-bold px-2 py-1 flex justify-between shrink-0'>
               ...
               <div className='text-yellow-300'>Customer Master</div>
            </div>
              ...
              {mode === 'list' ? (
                <>
                  <div className='flex justify-between items-center mb-2'>
                    <div className='font-bold text-slate-800 text-[14px]'>List of Customers</div>
                    <button 
                  ...
                  <table className='w-full text-left border-collapse border border-slate-400'>
                    <thead className='bg-[#eef5ed]'>
                      <tr className='border-b-2 border-slate-400 text-slate-900 font-bold text-[12px]'>
                        <th className="px-2 py-1 border-r border-slate-300 w-[50px] text-center">ID</th>
                        <th className="px-2 py-1 border-r border-slate-300">Customer Name</th>
                        <th className="px-2 py-1 border-r border-slate-300 w-[120px]">Phone</th>
                        <th className="px-2 py-1 border-r border-slate-300 w-[120px]">City</th>
                        <th className="px-2 py-1 border-r border-slate-300 w-[120px]">Loyalty Points</th>
                      </tr>
                    </thead>
                    <tbody>
                      {sampleData.map((row, idx) => {
                        ...
                        return (
                          <tr key={row.id} className={'text-[12px] border-b border-slate-300 ' + (idx % 2 === 0 ? 'bg-white' : 'bg-[#fcfaf2]') + ' hover:bg-[#ffffe0] cursor-pointer'}>
                            <td className="px-2 py-1 border-r border-slate-300 font-medium text-slate-700 text-center">{renderCell(0)}</td>
                            <td className="px-2 py-1 border-r border-slate-300 font-bold text-[#1b5e58]">{renderCell(1)}</td>
                            <td className="px-2 py-1 border-r border-slate-300 font-medium text-slate-700">{renderCell(2)}</td>
                            <td className="px-2 py-1 border-r border-slate-300 font-medium text-slate-700">{renderCell(3)}</td>
                            <td className="px-2 py-1 border-r border-slate-300 font-medium text-slate-700">{renderCell(4)}</td>
                          </tr>
                        );
                      })}
                    </tbody>
                ...
                <div className='flex flex-col h-full overflow-hidden'>
                  <div className='flex flex-1 gap-6 overflow-hidden'>
                    ...
                    <div className="w-[32%] flex flex-col gap-1 border-r-2 border-slate-300 pr-4 overflow-y-auto pb-4 custom-scrollbar">
                      ...
                      <InputRow label="Customer Name" value={formData.customerName} onChange={(v: string) => setFormData({...formData, customerName: v})} />
                      <InputRow label="Regional Name" value={formData.regionalName} onChange={(v: string) => setFormData({...formData, regionalName: v})} />
                      ...
                      <SectionTitle>Dates & Events</SectionTitle>
                      <InputRow label="Birth Date" type="date" value={formData.birthDate} onChange={(v: string) => setFormData({...formData, birthDate: v})} width="w-[110px]" />
                      <InputRow label="Anniversary" type="date" value={formData.anniversary} onChange={(v: string) => setFormData({...formData, anniversary: v})} width="w-[110px]" />
                    </div>

                    {/* Column 2: Legal & Local Address */}
                    <div className="w-[32%] flex flex-col gap-1 border-r-2 border-slate-300 pr-4 overflow-y-auto pb-4 custom-scrollbar">
                      ...
                      <SectionTitle>Local Address</SectionTitle>
                      ...
                      <InputRow label="Street" value={formData.street} onChange={(v: string) => setFormData({...formData, street: v})} />
                      <InputRow label="Village" value={formData.village} onChange={(v: string) => setFormData({...formData, village: v})} />
                      <InputRow label="Pincode" value={formData.pincode} onChange={(v: string) => setFormData({...formData, pincode: v})} width="w-[80px]" />
                    </div>

                    {/* Column 3: Region Details */}
                    <div className="flex-1 flex flex-col gap-1 pr-2 overflow-y-auto pb-4 custom-scrollbar">
                      <SectionTitle>Region Details</SectionTitle>
                      ...
                      <InputRow label="Country" value={formData.country} onChange={(v: string) => setFormData({...formData, country: v})} />
                    </div>
                  ...
                  <div className='flex justify-end gap-2 pt-2 border-t border-slate-300 mt-2 shrink-0'>
                    <button 
                      onClick={() => setFormData({
                        customerName: '', regionalName: '', mobile: '', email: '', gstin: '', birthDate: '', anniversary: '',
                        line1: '', line2: '', street: '', village: '', city: '', taluka: '', district: '', state: '', country: 'India', pincode: ''
                      })}
        ...
        <div className='bg-[#1b5e58] text-white text-[11px] px-4 py-1 flex justify-between items-center border-t-2 border-[#12423d]'>
          <div className='font-medium tracking-wide'>Customer Master</div>
        </div>
      </div>
    </>
  );
