...
export default function OrderInvoiceTable() {
  ...
  const sampleData = [
    { id: '1', orderNo: 'PO-2026-001', party: 'ABC Distributors', orderDate: '2026-08-01', items: 50, orderedBy: 'John Doe', ownedBy: 'Main Admin' },
    { id: '2', orderNo: 'PO-2026-002', party: 'XYZ Wholesalers', orderDate: '2026-08-02', items: 120, orderedBy: 'Jane Smith', ownedBy: 'Main Admin' },
  ];
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Escape') {
        ...
        e.preventDefault();
        if (sampleData[selectedRow]) {
          navigate(`/purchase/orders/invoice/${sampleData[selectedRow].id}`);
        }
      }
    };
  ...
  return (
    <>
      <Helmet>
        <title>Order Invoices | RetailNode ERP</title>
      </Helmet>
      ...
      <div className='flex flex-col h-screen font-sans text-[13px] selection:bg-transparent overflow-hidden bg-[#e0efeb] w-full'>
        ...
        <div className='flex flex-1 p-1 gap-1 overflow-hidden h-full'>
          ...
          <div className='flex-1 bg-[#fcfaf2] border-2 border-[#81a09d] flex flex-col overflow-hidden shadow-inner relative'>
            <div className='bg-[#1b5e58] text-white font-bold px-2 py-1 flex justify-between shrink-0'>
               ...
               <div className='text-yellow-300'>Invoices List</div>
            </div>
            
            <div className='p-2 flex-1 flex flex-col overflow-hidden'>
              
              <div className="flex justify-between items-center mb-2">
                 <div className="font-bold text-[14px]">List of {status} Purchase Orders</div>
                ...
                <table className='w-full text-left border-collapse'>
                  <thead className='bg-[#eef5ed] sticky top-0'>
                    <tr className='border-b-2 border-slate-400 text-slate-900 font-bold text-[12px]'>
                      ...
                      <th className="px-2 py-1 border-r border-slate-300">Order No</th>
                      <th className="px-2 py-1 border-r border-slate-300">Party</th>
                      <th className="px-2 py-1 border-r border-slate-300">Order Date</th>
                      <th className="px-2 py-1 border-r border-slate-300 text-center">Total Items</th>
                      <th className="px-2 py-1 border-r border-slate-300">Ordered By</th>
                      <th className="px-2 py-1">Owned By</th>
                    </tr>
                  </thead>
                  <tbody>
                    {sampleData.length === 0 ? (
                      <tr>
                        <td colSpan={7} className="text-center py-4 font-medium text-slate-500">No orders found.</td>
                      </tr>
                    ) : (
                      sampleData.map((row, idx) => (
                        ...
                        >
                          ...
                          <td className="px-2 py-1 border-r border-slate-300 text-blue-700 font-bold">{row.orderNo}</td>
                          <td className="px-2 py-1 border-r border-slate-300 text-slate-900">{row.party}</td>
                          <td className="px-2 py-1 border-r border-slate-300">{row.orderDate}</td>
                          <td className="px-2 py-1 border-r border-slate-300 text-center font-bold">{row.items}</td>
                          <td className="px-2 py-1 border-r border-slate-300">{row.orderedBy}</td>
                          <td className="px-2 py-1">{row.ownedBy}</td>
                        </tr>
                      ))
                    )}
                  </tbody>
        ...
        <div className='bg-[#1b5e58] text-white text-[11px] px-4 py-1 flex justify-between items-center border-t-2 border-[#12423d]'>
          <div className='font-medium tracking-wide'>Order Invoices</div>
          <div className='font-medium tracking-wide'>{sampleData.length} Invoice(s)</div>
        </div>
      </div>
    </>
  );
