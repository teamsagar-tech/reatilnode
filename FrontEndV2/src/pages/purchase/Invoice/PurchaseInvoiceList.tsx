    ...
    .then(data => {
      if (Array.isArray(data)) {
        const mappedData = data.map(inv => ({
          ...
          discount: inv.discount_amount ? Number(inv.discount_amount).toLocaleString('en-IN', { minimumFractionDigits: 2 }) : '',
          addChgs: inv.commission_amount ? Number(inv.commission_amount).toLocaleString('en-IN', { minimumFractionDigits: 2 }) : '',
          taxableAmt: (inv.total_amount || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 }),
          gstPercent: '',
          cgstAmt: inv.gst_amount ? (Number(inv.gst_amount) / 2).toLocaleString('en-IN', { minimumFractionDigits: 2 }) : '',
          sgstAmt: inv.gst_amount ? (Number(inv.gst_amount) / 2).toLocaleString('en-IN', { minimumFractionDigits: 2 }) : '',
          igstAmt: '',
          addLess: '',
          rOff: '',
