...
const InputRow = ({ label, value, onChange, width = 'flex-1', type = 'text', placeholder = '', autoFocus = false }: any) => (
...
export default function UserMaster() {
  ...
  const [mode, setMode] = useState<'list' | 'create_user' | 'edit_user' | 'create_series' | 'edit_series'>('list');
  const [focusIndex, setFocusIndex] = useState(0);
  
  const [userForm, setUserForm] = useState<any>({ id: null, name: '', email: '', mobile_no: '', password: '', employee_id: '', role: 'user', role_id: null, locations: [] });
  const [isSystemUser, setIsSystemUser] = useState<boolean>(true);
  const [seriesForm, setSeriesForm] = useState({ id: null as any, series_name: '', start_num: '', end_num: '' });

  const fetchData = async () => {
    try {
      const [usersRes, seriesRes, locRes, rolesRes] = await Promise.all([
      ...
      if (usersRes.ok) {
        try { setUsers(await usersRes.json()); } catch(e) {}
      }
      if (seriesRes.ok) {
        try { setSeriesList(await seriesRes.json()); } catch(e) {}
      }
      if (locRes.ok) {
        try { setLocations(await locRes.json()); } catch(e) {}
      }
      if (rolesRes.ok) {
        try { setRoles(await rolesRes.json()); } catch(e) {}
      }
    } catch (err) {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (mode === 'list') {
        const totalRows = users.length + seriesList.length;
        if (e.key === 'ArrowDown') {
          e.preventDefault();
          setFocusIndex(prev => Math.min(prev + 1, Math.max(0, totalRows - 1)));
          ...
          if (totalRows > 0) {
            if (focusIndex < users.length) {
              ...
              const s = seriesList[focusIndex - users.length];
              if (s) {
                setSeriesForm({ ...s });
                setMode('edit_series');
              }
  ...
  }, [navigate, mode, users, seriesList, focusIndex]);

  useEffect(() => {
    if (mode === 'list') {
      const el = document.getElementById(`row-${focusIndex}`);
  ...
  const handleSaveUser = async () => {
    ...
    // Assign a dummy internal email if none provided (required by unique constraint if not null, though MySQL allows multiple nulls)
    // We will handle this in the backend, so we just pass what we have.
      ...
      <div className='flex flex-col h-screen font-sans text-[13px] selection:bg-transparent overflow-hidden bg-[#e0efeb] w-full'>
        <div className='flex flex-1 p-1 gap-1 overflow-hidden h-full'>
          ...
          <div className='flex-1 bg-[#fcfaf2] border-2 border-[#81a09d] flex flex-col overflow-hidden shadow-inner relative'>
            ...
            <div 
              className='p-2 flex-1 flex flex-col overflow-y-auto outline-none'
              ref={containerRef}
            >
              {mode === 'list' ? (
                <>
                  <div className="flex justify-between items-center mb-2 pb-1 border-b border-[#1b5e58]">
                    ...
                    <button onClick={() => { setMode('create_user'); setUserForm({ id: null, name: '', email: '', mobile_no: '', password: '', employee_id: '', role: 'user', role_id: null, locations: [] }); }} className="bg-[#eef5ed] border border-[#a3c3be] px-3 py-1 font-bold text-black shadow-[inset_1px_1px_0_rgba(255,255,255,0.8)] hover:bg-[#ffe000] text-[12px]">Create New User (Alt+C)</button>
                  </div>
                    ...
                    <tbody>
                      {users.map((u, index) => (
                        <tr 
                          key={u.id} 
                          id={`row-${index}`}
                          className={`border-b border-slate-300 cursor-pointer ${focusIndex === index ? 'bg-[#ffe000]' : 'hover:bg-[#ffffe0]'}`}
                          onClick={() => setFocusIndex(index)}
                          onDoubleClick={() => {
                            setFocusIndex(index);
                            setUserForm({ ...u, password: '', locations: u.locations || [], role_id: u.role_id || null });
                            setIsSystemUser(!u.email || !u.email.includes('@internal.local'));
                            setMode('edit_user');
                          }}
                        >
                          <td className="px-2 py-1 border-r border-slate-300">{u.name}</td>
                          <td className="px-2 py-1 border-r border-slate-300">{u.email?.includes('@internal.local') ? '-' : u.email}</td>
                  ...
                  <table className="w-full text-left border-collapse bg-white border border-slate-300 w-1/2 min-w-[400px]">
                    ...
                    <tbody>
                      {seriesList.map((s, index) => (
                        <tr 
                          key={s.id} 
                          id={`row-${users.length + index}`}
                          className={`border-b border-slate-300 cursor-pointer ${focusIndex === users.length + index ? 'bg-[#ffe000]' : 'hover:bg-[#ffffe0]'}`}
                          onClick={() => setFocusIndex(users.length + index)}
                          onDoubleClick={() => {
                            setFocusIndex(users.length + index);
                            setSeriesForm({ ...s });
                            setMode('edit_series');
                          }}
                        >
                ...
                <div className="w-[500px]">
                  ...
                  <div className="flex flex-row items-center mb-1">
                    <div className="w-[110px] text-right pr-2 text-slate-800 font-bold text-[12px]">User Type:</div>
                    <select
                      className="flex-1 border border-slate-400 px-1 py-1 bg-white focus:bg-[#ffffe0] focus:outline-none focus:border-[#1b5e58] focus:ring-1 focus:ring-[#1b5e58] text-[13px]"
                      value={isSystemUser ? 'system' : 'non-system'}
                      onChange={(e) => {
                        setIsSystemUser(e.target.value === 'system');
                        if (e.target.value === 'non-system') {
                          setUserForm({...userForm, email: '', password: ''});
                        }
                      }}
                    >
                      <option value="system">System User (Can Login)</option>
                      <option value="non-system">Non-System User (Employee Only)</option>
                    </select>
                  </div>

                  <div className="flex flex-row items-center mb-1">
                    <div className="w-[110px] text-right pr-2 text-slate-800 font-bold text-[12px]">Role:</div>
                    <select
                      className="flex-1 border border-slate-400 px-1 py-1 bg-white focus:bg-[#ffffe0] focus:outline-none focus:border-[#1b5e58] focus:ring-1 focus:ring-[#1b5e58] text-[13px]"
                      value={userForm.role === 'admin' ? 'admin' : (userForm.role_id ? `custom_${userForm.role_id}` : 'user')}
                      onChange={(e) => {
                        const val = e.target.value;
                        if (val === 'admin') {
                          setUserForm({...userForm, role: 'admin', role_id: null});
                        } else if (val === 'user') {
                          setUserForm({...userForm, role: 'user', role_id: null});
                        } else if (val.startsWith('custom_')) {
                          setUserForm({...userForm, role: 'user', role_id: parseInt(val.replace('custom_', ''))});
                        }
                      }}
                    >
                      <optgroup label="System Roles">
                        <option value="admin">Administrator (Access to All)</option>
                        <option value="user">Standard User</option>
                      </optgroup>
                      <optgroup label="Group Roles">
                        {roles.map(r => (
                  ...
                  <InputRow label="Name:" autoFocus value={userForm.name || ''} onChange={(v: string) => setUserForm({...userForm, name: v})} />
                  
                  {isSystemUser && (
                    <>
                      <InputRow label="Username / Email:" type="email" value={userForm.email || ''} onChange={(v: string) => setUserForm({...userForm, email: v})} />
                      {mode === 'create_user' && (
                  ...
                  {userForm.role !== 'admin' && (
                    <div className="flex flex-row mb-1 mt-2">
                      <div className="w-[110px] text-right pr-2 text-slate-800 font-bold text-[12px] pt-1">Assigned Locations:</div>
                      <div className="flex-1 border border-slate-400 p-2 bg-white max-h-[150px] overflow-y-auto">
                        {locations.length === 0 ? (
                          <div className="text-gray-400 italic text-[11px]">No locations found</div>
                        ) : (
                          locations.map(loc => {
                            const isSelected = (userForm.locations || []).includes(loc.id);
                            return (
                              <label key={loc.id} className="flex items-center gap-2 mb-1 cursor-pointer hover:bg-gray-100 p-1">
                                <input 
                                  type="checkbox" 
                                  checked={isSelected}
                                  onChange={(e) => {
                                    const newLocs = e.target.checked 
                                      ? [...(userForm.locations || []), loc.id]
                                      : (userForm.locations || []).filter((id: number) => id !== loc.id);
                                    setUserForm({ ...userForm, locations: newLocs });
                                  }}
                                  className="accent-[#1b5e58]"
                                />
                                <span className="text-[12px] text-slate-800">{loc.name}</span>
                              </label>
                            );
                          })
                        )}
                ...
                <div className="w-[400px]">
                  ...
                  <InputRow label="Series Name:" autoFocus placeholder="e.g. Admin, First Floor" value={seriesForm.series_name || ''} onChange={(v: string) => setSeriesForm({...seriesForm, series_name: v})} />
