...
export default function ChargesTypeMaster() {
  ...
  return (
    <>
      <Helmet>
        <title>ChargesType Master | RetailNode ERP</title>
      </Helmet>
      ...
      <div className='flex flex-col h-screen font-sans text-[13px] selection:bg-transparent overflow-hidden bg-[#e0efeb] w-full'>
        ...
        <div className='flex flex-1 p-1 gap-1 overflow-hidden h-full'>
          ...
          <div className='flex-1 bg-[#fcfaf2] border-2 border-[#81a09d] flex flex-col overflow-hidden shadow-inner relative'>
            <div className='bg-[#1b5e58] text-white font-bold px-2 py-1 flex justify-between shrink-0'>
               ...
               <div className='text-yellow-300'>ChargesType Master</div>
            </div>
              ...
              {mode === 'list' ? (
                <>
                  <div className='flex justify-between items-center mb-2'>
                    <div className='font-bold text-slate-800 text-[14px]'>List of ChargesTypes</div>
                    <button 
                  ...
                  <table className='w-full text-left border-collapse border border-slate-400'>
                    <thead className='bg-[#eef5ed]'>
                      <tr className='border-b-2 border-slate-400 text-slate-900 font-bold text-[12px]'>
                        <th className="px-2 py-1 border-r border-slate-300">ID</th><th className="px-2 py-1 border-r border-slate-300">Charge Name</th><th className="px-2 py-1 border-r border-slate-300">Default Amount</th><th className="px-2 py-1 border-r border-slate-300">Type</th>
                      </tr>
                    </thead>
                    <tbody>
                ...
                <div className='flex flex-col h-full overflow-hidden'>
                  <div className='flex flex-1 gap-6 overflow-hidden'>
                    ...
                    <InputRow label="Charge Name" value={formData.chargeName} onChange={(v: string) => setFormData({...formData, chargeName: v})} />
                    <InputRow label="Default Amount" value={formData.defaultAmount} onChange={(v: string) => setFormData({...formData, defaultAmount: v})} />
                    <InputRow label="Type" value={formData.type} onChange={(v: string) => setFormData({...formData, type: v})} />
                    </div>
        ...
        <div className='bg-[#1b5e58] text-white text-[11px] px-4 py-1 flex justify-between items-center border-t-2 border-[#12423d]'>
          <div className='font-medium tracking-wide'>ChargesType Master</div>
        </div>
      </div>
    </>
  );
