...
export default function ColorMaster() {
  ...
  const handleDeleteColor = async () => {
    if (!deleteId) return;
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/masters/generic/colors/${deleteId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      });
      if (res.ok) {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Escape') {
        ...
        if (e.key === 'ArrowDown') {
          e.preventDefault();
          setSelectedIndex(prev => Math.min(prev + 1, colors.length - 1));
          ...
          if (colors[selectedIndex]) {
            setDeleteId(colors[selectedIndex].id);
            setShowDeleteConfirm(true);
          }
        } else if (e.key === 'Enter') {
          e.preventDefault();
          if (colors[selectedIndex]) {
            const row = colors[selectedIndex];
            setFormData({
              name: row.name,
  ...
  }, [navigate, mode, formData, colors, selectedIndex, showResetConfirm, showDeleteConfirm]);

  return (
    <>
      <Helmet>
        <title>Color Master | RetailNode ERP</title>
      </Helmet>
      ...
      <div className='flex flex-col h-screen font-sans text-[13px] selection:bg-transparent overflow-hidden bg-[#e0efeb] w-full'>
        ...
        <div className='flex flex-1 p-1 gap-1 overflow-hidden h-full'>
          ...
          <div className='flex-1 bg-[#fcfaf2] border-2 border-[#81a09d] flex flex-col overflow-hidden shadow-inner relative'>
            <div className='bg-[#1b5e58] text-white font-bold px-2 py-1 flex justify-between shrink-0'>
               ...
               <div className='text-yellow-300'>Color Master</div>
            </div>
              ...
              {mode === 'list' ? (
                <>
                  <div className='flex justify-between items-center mb-2'>
                    <div className='font-bold text-slate-800 text-[14px]'>List of Colors</div>
                    <button 
                  ...
                  <table className='w-full text-left border-collapse border border-slate-400'>
                    <thead className='bg-[#eef5ed]'>
                      <tr className='border-b-2 border-slate-400 text-slate-900 font-bold text-[12px]'>
                        <th className="px-2 py-1 border-r border-slate-300">ID</th><th className="px-2 py-1 border-r border-slate-300">Color Name</th><th className="px-2 py-1 border-r border-slate-300">Description</th>
                      </tr>
                    </thead>
                    <tbody>
                      {colors.map((row, idx) => {
                ...
                <div className='flex flex-col h-full overflow-hidden'>
                  <div className='flex flex-1 gap-6 overflow-hidden'>
                    ...
                    <div className="w-[40%] flex flex-col gap-1 border-r-2 border-slate-300 pr-4 overflow-y-auto pb-4 custom-scrollbar">
                      ...
                      <InputRow id="input-name" label="Color Name" value={formData.name} onChange={(v: string) => setFormData({...formData, name: v})} />
        ...
        <div className='bg-[#1b5e58] text-white text-[11px] px-4 py-1 flex justify-between items-center border-t-2 border-[#12423d]'>
          <div className='font-medium tracking-wide'>Color Master</div>
        </div>
        <ConfirmModal 
