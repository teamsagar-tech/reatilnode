...
export default function DepartmentMaster() {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Escape') {
        ...
        if (e.key === 'ArrowDown') {
          e.preventDefault();
          setSelectedIndex(prev => Math.min(prev + 1, departments.length - 1));
          ...
          if (departments[selectedIndex]) {
            const row = departments[selectedIndex];
            setFormData({
              name: row.name,
  ...
  return (
    <>
      <Helmet>
        <title>Department Master | RetailNode ERP</title>
      </Helmet>
        ...
        <div className='flex flex-1 p-1 gap-1 overflow-hidden h-full'>
          ...
          <div className='flex-1 bg-[#fcfaf2] border-2 border-[#81a09d] flex flex-col overflow-hidden shadow-inner relative'>
            <div className='bg-[#1b5e58] text-white font-bold px-2 py-1 flex justify-between shrink-0'>
               ...
               <div className='text-yellow-300'>Department Master</div>
            </div>
              ...
              {mode === 'list' ? (
                <>
                  <div className='flex justify-between items-center mb-2'>
                    <div className='font-bold text-slate-800 text-[14px]'>List of Departments</div>
                    <button 
                  ...
                  <table className='w-full text-left border-collapse border border-slate-400'>
                    <thead className='bg-[#eef5ed]'>
                      <tr className='border-b-2 border-slate-400 text-slate-900 font-bold text-[12px]'>
                        <th className="px-2 py-1 border-r border-slate-300">ID</th><th className="px-2 py-1 border-r border-slate-300">Department Name</th><th className="px-2 py-1 border-r border-slate-300">Description</th>
                      </tr>
                    </thead>
                    <tbody>
                ...
                <div className='flex flex-col h-full overflow-hidden'>
                  <div className='flex flex-1 gap-6 overflow-hidden'>
                    ...
                    <div className="w-[40%] flex flex-col gap-1 border-r-2 border-slate-300 pr-4 overflow-y-auto pb-4 custom-scrollbar">
                      ...
                      <InputRow id="input-name" label="Department Name" value={formData.name} onChange={(v: string) => setFormData({...formData, name: v})} />
