...
export default function CutMaster() {
  ...
  const handleSaveCut = async () => {
    ...
    try {
      ...
      const url = editId ? `${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/masters/cut/${editId}` : `${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/masters/cut`;
      ...
      const res = await fetch(url, {
        ...
        body: JSON.stringify({ 
          cut_name: formData.cut_name.toUpperCase(), 
          cut_size: parseFloat(formData.cut_size) 
        })
      });
      if (res.ok) {
        setFormData({});
        ...
        let errText = 'Failed to save cut';
        try {
  ...
  const handleDeleteCut = async () => {
    if (!deleteId) return;
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/masters/cut/${deleteId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      });
      if (res.ok) {
  ...
  useEffect(() => {
    if (mode === 'create') {
      setTimeout(() => {
        document.getElementById('input-cutName')?.focus();
      }, 50);
    }
  }, [mode]);
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Escape') {
        ...
        if (e.key === 'ArrowDown') {
          e.preventDefault();
          setSelectedIndex(prev => Math.min(prev + 1, cuts.length - 1));
        } else if (e.key === 'ArrowUp') {
          e.preventDefault();
          setSelectedIndex(prev => Math.max(prev - 1, 0));
        } else if (e.key === 'F5') {
          e.preventDefault();
          if (cuts[selectedIndex]) {
            setDeleteId(cuts[selectedIndex].id);
            setShowDeleteConfirm(true);
          }
        } else if (e.key === 'Enter') {
          e.preventDefault();
          if (cuts[selectedIndex]) {
            const row = cuts[selectedIndex];
            setFormData({
  ...
  }, [navigate, mode, formData, cuts, selectedIndex, showResetConfirm, showDeleteConfirm]);

  return (
    <>
      <Helmet>
        <title>Cut Master | RetailNode ERP</title>
      </Helmet>
      ...
      <div className='flex flex-col h-screen font-sans text-[13px] selection:bg-transparent overflow-hidden bg-[#e0efeb] w-full'>
        ...
        <div className='flex flex-1 p-1 gap-1 overflow-hidden h-full'>
          ...
          <div className='flex-1 bg-[#fcfaf2] border-2 border-[#81a09d] flex flex-col overflow-hidden shadow-inner relative'>
            <div className='bg-[#1b5e58] text-white font-bold px-2 py-1 flex justify-between shrink-0'>
               ...
               <div className='text-yellow-300'>Cut Master</div>
            </div>
              ...
              {mode === 'list' ? (
                <>
                  <div className='flex justify-between items-center mb-2'>
                    <div className='font-bold text-slate-800 text-[14px]'>List of Cuts</div>
                    <button 
                  ...
                  <table className='w-full text-left border-collapse border border-slate-400'>
                    <thead className='bg-[#eef5ed]'>
                      <tr className='border-b-2 border-slate-400 text-slate-900 font-bold text-[12px]'>
                        ...
                        <th className="px-2 py-1 border-r border-slate-300 w-40">Cut Size (Mtrs)</th>
                        <th className="px-2 py-1 border-r border-slate-300 w-24">Active</th>
                      </tr>
                    </thead>
                    <tbody>
                      {cuts.map((row, idx) => {
                        return (
                          ...
                          >
                            <td className="px-2 py-1 border-r border-slate-300 font-medium text-slate-700">{idx + 1}</td>
                            <td className="px-2 py-1 border-r border-slate-300 font-medium text-slate-700">{row.cut_name}</td>
                            <td className="px-2 py-1 border-r border-slate-300 font-medium text-slate-700">{row.cut_size}</td>
                      ...
                      {cuts.length === 0 && (
                        <tr>
                          <td colSpan={4} className="px-2 py-4 text-center text-slate-500 font-bold">No cuts found. Press Alt+C to create.</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
                ...
                <div className='flex flex-col h-full overflow-hidden'>
                  <div className='flex flex-1 gap-6 overflow-hidden'>
                    ...
                    <div className="w-[40%] flex flex-col gap-1 border-r-2 border-slate-300 pr-4 overflow-y-auto pb-4 custom-scrollbar">
                      ...
                      <InputRow id="input-cutName" label="Cut Name" value={formData.cut_name} onChange={(v: string) => setFormData({...formData, cut_name: v})} placeholder="e.g. SUITING STD" />
                      <InputRow id="input-cutSize" type="number" label="Cut Size (Mtrs)" value={formData.cut_size} onChange={(v: string) => setFormData({...formData, cut_size: v})} placeholder="e.g. 1.20" />
                    </div>
        ...
        <div className='bg-[#1b5e58] text-white text-[11px] px-4 py-1 flex justify-between items-center border-t-2 border-[#12423d]'>
          <div className='font-medium tracking-wide'>Cut Master</div>
        </div>
