...
export default function TransporterMaster() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (data.success) {
        setMode('list');
        setFormData({ transporter_name: '', mobile: '', email: '' });
      } else {
    ...
    <>
      ...
      <div className='flex flex-col h-screen font-sans text-[13px] selection:bg-transparent overflow-hidden bg-[#e0efeb] w-full'>
                  ...
                  <table className='w-full text-left border-collapse border border-slate-400'>
                    <thead className='bg-[#eef5ed]'>
                      <tr className='border-b-2 border-slate-400 text-slate-900 font-bold text-[12px]'>
                        ...
                        <th className="px-2 py-1 border-r border-slate-300">Transporter Name</th>
                    ...
                    <tbody>
                      {transporters.length === 0 ? (
                        <tr><td colSpan={4} className="text-center p-4 text-slate-500">No transporters found</td></tr>
                      ) : (
        ...
        <ConfirmModal
          ...
          type="warning"
          onConfirm={() => {
            const resetFn = () => setFormData({ transporter_name: '', mobile: '', email: '' });
            resetFn();
